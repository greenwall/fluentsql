...
public class SqlStatement {
	...
	private final List<AddParam> addParams = new ArrayList<AddParam>();
	...
	public SqlStatement addObject(final Object param) {
		if (param instanceof byte[]) {
			return addBinaryStream((byte[]) param);
		} else {
			addParams.add(new AddParam() {
				@Override
				public void addTo(PreparedStatement ps, int index) throws SQLException {				
...
/*	
	public <T> T exactQuery(Connection con, JdbcUtil.RowMapper<T> rowMapper) throws SQLException {		
		try (PreparedStatement ps = prepareStatement(con)) {							
			return JdbcUtil.exactQuery(ps, rowMapper);
		}
	}
*/	
